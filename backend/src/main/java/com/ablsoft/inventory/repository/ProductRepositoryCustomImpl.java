...
/**
 * The SQL behind {@link ProductRepositoryCustom}.
 *
 * <p>Spring Data wires this in automatically because of the {@code Impl} suffix on the fragment
 * interface's name, so callers still see one {@link ProductRepository}.
 */
@Repository
public class ProductRepositoryCustomImpl implements ProductRepositoryCustom {
    ...
    /**
     * Keyset pagination: fetch the rows after the cursor, in sort order.
     *
     * <p>The tiebreaker on {@code id} is what makes this correct on a non-unique column. Without
     * it, two rows sharing a price would sit either side of a page boundary in an order the
     * database is free to change between queries, and one of them would be skipped or repeated.
     *
     * <p>One row more than asked for is fetched, purely to learn whether another page exists
    ...
    /** A mapped row plus the id the cursor needs but the API does not expose. */
    ...
    /**
     * The value the next cursor resumes from. It must be the value the database ordered by, which
     * is why stock age reports its purchase date rather than the age computed in Java.
     */
